...
class ECEformerScorer(RelationalScorer):
    ...
    def _get_encoder_output(self, e_emb, p_emb, t_emb, ids, gt_ent, gt_rel, gt_tim, t_ids, output_repr=False):
        ...
        # 根据 ids (if "sp_" ids='s',if "_po" ids='o') 取出对应邻居
        ...
        if self.training and self.get_option(
                "self_dropout") > 0 and self.max_context_size > 0 and not output_repr and self.get_option(
                "add_mlm_loss"):
        ...
        if self.training and self.get_option("add_mlm_loss") and self.get_option(
                "self_dropout") > 0.0 and self_dropout_sample.sum() > 0:
    ...
    def _scoring(self, s_emb, p_emb, o_emb, t_emb, is_pairwise, ids, gt_ent, gt_rel, gt_tim, t_ids):
        encoder_output, self_pred_loss = self._get_encoder_output(s_emb, p_emb, t_emb, ids, gt_ent, gt_rel, gt_tim,
                                                                  t_ids)
    ...
    def score_emb(self, s_emb, p_emb, o_emb, t_emb, combine: str, s, o, gt_ent=None, gt_rel=None, gt_tim=None,
                  t_ids=None):
...
class ECEformer(KgeModel):
    ...
    def score_sp(self, s: Tensor, p: Tensor, t: Tensor, o: Tensor = None, gt_ent=None, gt_rel=None,
                 gt_tim=None) -> Tensor:
        s_emb = self.get_s_embedder().embed(s)
        p_emb = self.get_p_embedder().embed(p)
        t_emb = self.get_t_embedder().embed(t)
    ...
    def score_po(self, p: Tensor, o: Tensor, t: Tensor, s: Tensor = None, gt_ent=None, gt_rel=None,
                 gt_tim=None) -> Tensor:
        if s is None:
